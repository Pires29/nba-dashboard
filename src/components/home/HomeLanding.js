...
export default function HomeLanding() {
  return (
    <div className="overflow-hidden scroll-smooth bg-[#060b13] text-white">
      ...
      <section id="features" className="landing-deferred-content relative scroll-mt-20 border-y border-white/[0.06] bg-[#09111d]/80 py-24">
      ...
      <FaqSection />
      <section className="landing-deferred-content relative px-6 py-24 text-center"><div className="absolute left-1/2 top-1/2 h-52 w-96 -translate-x-1/2 -translate-y-1/2 rounded-full bg-orange-500/10 blur-[100px]" /><div className="relative"><p className="font-mono text-[10px] uppercase tracking-[0.25em] text-orange-400">Your next slate starts here</p><h2 className="mx-auto mt-5 max-w-3xl text-4xl font-black tracking-tight sm:text-6xl">Stop guessing. Start researching.</h2><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><PrimaryCta className="inline-flex items-center justify-center rounded-xl bg-orange-500 px-7 py-3.5 font-mono text-xs font-black uppercase leading-none tracking-widest hover:bg-orange-400" /><PricingLink className="inline-flex items-center justify-center rounded-xl border border-white/10 px-7 py-3.5 font-mono text-xs font-bold uppercase leading-none tracking-widest text-slate-300 hover:border-white/20">{launchConfig.cta.secondary}</PricingLink></div></div></section>
      <footer className="landing-deferred-content border-t border-white/[0.06]"><div className="mx-auto flex max-w-[1320px] flex-wrap items-center justify-between gap-5 px-6 py-7"><p className="font-mono text-[10px] text-slate-400">© {new Date().getFullYear()} PropInsight. Research responsibly.</p><div className="flex gap-5 font-mono text-[9px] uppercase tracking-widest text-slate-400"><a href="#features" className="hover:text-slate-300">Features</a><PricingLink className="hover:text-slate-300">Pricing</PricingLink><a href="#faq" className="hover:text-slate-300">FAQ</a><Link href="/privacy" className="hover:text-slate-300">Privacy</Link><Link href="/terms" className="hover:text-slate-300">Terms</Link></div></div></footer>
    </div>
  );
}

function FaqSection() {
  return <section id="faq" className="landing-deferred-content scroll-mt-20 border-t border-white/[0.06] bg-[#09111d]/75 py-24"><div className="mx-auto max-w-3xl px-6"><div className="text-center"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.25em] text-orange-400">FAQ</p><h2 className="mt-4 text-4xl font-black">Good questions. Clear answers.</h2></div><div className="mt-12 space-y-3">{ALL_FAQS.map((faq, index) => <details key={faq.id} className="group rounded-xl border border-white/[0.07] bg-[#0b1421] p-5 open:border-orange-500/20"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-sm font-bold"><span><span className="mr-4 font-mono text-[9px] text-orange-400">0{index + 1}</span>{faq.question}</span><span className="font-mono text-lg font-light text-slate-500 transition group-open:rotate-45">+</span></summary><p className="ml-9 mt-4 max-w-2xl text-sm leading-6 text-slate-400">{faq.answer}</p></details>)}</div></div></section>;
}
