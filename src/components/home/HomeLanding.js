...
const PLANS = [
  {
    ...
    details: ["Nothing charged today", "7 days of full Pro access", "Then €3.99 per month", "Cancel before the trial ends"],
  },
  {
    id: "monthly",
    name: "Monthly",
  ...
  },
  {
    ...
    details: ["Single payment of €29.99", "No automatic renewal", "Access until June 30", "Best value for the full season"],
  },
];
const PREVIEW_ROWS = [
  { player: "J. Brunson", team: "NYK", line: "28.5", l5: "80%", l10: "70%", matchup: "Favorable" },
  { player: "S. Gilgeous-Alexander", team: "OKC", line: "31.5", l5: "60%", l10: "80%", matchup: "Neutral" },
  { player: "N. Jokic", team: "DEN", line: "25.5", l5: "80%", l10: "70%", matchup: "Favorable" },
  { player: "A. Edwards", team: "MIN", line: "26.5", l5: "60%", l10: "60%", matchup: "Tough" },
];
const BENEFITS = [
    ...
    <div className="overflow-hidden bg-[#060b13] text-white">
      ...
      <section className="relative mx-auto max-w-[1240px] px-6 pb-20 pt-20 sm:pt-28 lg:pb-28">
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          <h1 className="text-5xl font-black leading-[1.12] tracking-normal sm:text-7xl sm:leading-[1.08] lg:text-[84px]">Research every prop.<span className="block pb-3 bg-gradient-to-r from-orange-400 via-amber-300 to-orange-500 bg-clip-text text-transparent">Find your edge.</span></h1>
          ...
          <div className="mx-auto mt-10 max-w-3xl overflow-hidden rounded-2xl border border-amber-400/30 bg-gradient-to-r from-amber-500/[0.12] via-orange-500/[0.08] to-[#0b1421] shadow-[0_18px_55px_rgba(249,115,22,.12)]">
            ...
            <div className="grid gap-6 p-6 sm:p-7 md:grid-cols-[1fr_auto] md:items-center">
              <div>
                ...
                <p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">Your trial includes full Pro access. It will renew automatically at <strong className="text-white">€3.99/month</strong>{trialEndsAt ? <> on <strong className="text-white">{trialEndsAt}</strong></> : null} unless you cancel before then.</p>
              </div>
      ...
      <footer className="border-t border-white/[0.06]"><div className="mx-auto flex max-w-[1120px] flex-wrap items-center justify-between gap-5 px-6 py-7"><p className="font-mono text-[10px] text-slate-700">© {new Date().getFullYear()} HoopiQ. Research responsibly.</p><div className="flex gap-5 font-mono text-[9px] uppercase tracking-widest text-slate-600"><a href="#features" className="hover:text-slate-300">Features</a><a href="#pricing" className="hover:text-slate-300">Pricing</a><a href="#faq" className="hover:text-slate-300">FAQ</a><Link href="/privacy" className="hover:text-slate-300">Privacy</Link><Link href="/terms" className="hover:text-slate-300">Terms</Link></div></div></footer>
    </div>
  );
}

function ProductPreview() {
  return <div className="relative mx-auto mt-16 max-w-5xl rounded-2xl border border-white/10 bg-[#0c1421]/95 p-2 shadow-[0_35px_100px_rgba(0,0,0,.45)] sm:p-4"><div className="absolute -inset-px -z-10 rounded-2xl bg-gradient-to-r from-orange-500/30 via-transparent to-amber-500/20 blur-sm" /><div className="overflow-hidden rounded-xl border border-white/[0.07] bg-[#0a111d]"><div className="flex items-center justify-between border-b border-white/[0.06] px-4 py-3 sm:px-6"><div><p className="font-mono text-[9px] uppercase tracking-[0.2em] text-slate-600">Tonight&apos;s board</p><p className="mt-1 text-sm font-black">Player Props <span className="ml-2 text-orange-400">Points</span></p></div><div className="hidden gap-2 sm:flex"><span className="rounded-md border border-white/[0.07] px-3 py-1.5 font-mono text-[9px] text-slate-500">All games</span><span className="rounded-md bg-orange-500/10 px-3 py-1.5 font-mono text-[9px] text-orange-400">L10 hit rate ↓</span></div></div><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-left"><thead className="font-mono text-[9px] uppercase tracking-widest text-slate-600"><tr className="border-b border-white/[0.05]"><th className="px-6 py-3">Player</th><th>Line</th><th>L5</th><th>L10</th><th>Matchup</th><th className="pr-6 text-right">Research</th></tr></thead><tbody>{PREVIEW_ROWS.map((row) => <tr key={row.player} className="border-b border-white/[0.04] last:border-0"><td className="px-6 py-4"><p className="text-xs font-bold text-slate-200">{row.player}</p><p className="mt-0.5 font-mono text-[9px] text-slate-600">{row.team} · POINTS</p></td><td className="font-mono text-xs text-white">{row.line}</td><td className="font-mono text-xs font-bold text-emerald-400">{row.l5}</td><td className="font-mono text-xs font-bold text-emerald-400">{row.l10}</td><td><span className={`rounded-md px-2 py-1 font-mono text-[9px] ${row.matchup === "Tough" ? "bg-red-500/10 text-red-400" : row.matchup === "Neutral" ? "bg-amber-500/10 text-amber-300" : "bg-emerald-500/10 text-emerald-400"}`}>{row.matchup}</span></td><td className="pr-6 text-right font-mono text-[10px] text-orange-400">View →</td></tr>)}</tbody></table></div></div></div>;
}

function ProPlan() {
  return <div className="mx-auto mt-12 max-w-2xl rounded-2xl border border-emerald-500/25 bg-emerald-500/[0.05] p-8 text-center"><p className="font-mono text-[10px] uppercase tracking-widest text-emerald-400">Your Pro plan is active</p><h3 className="mt-3 text-2xl font-black">You already have the full edge.</h3><p className="mt-3 text-sm text-slate-400">Every NBA player and every HoopiQ research feature is available to you.</p><div className="mt-7 flex justify-center gap-3"><Link href="/props" className="rounded-xl bg-orange-500 px-6 py-3 font-mono text-[11px] font-bold uppercase tracking-widest">Open Props</Link><Link href="/settings" className="rounded-xl border border-white/10 px-6 py-3 font-mono text-[11px] uppercase tracking-widest text-slate-300">Manage Plan</Link></div></div>;
}
