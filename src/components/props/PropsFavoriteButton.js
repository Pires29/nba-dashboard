...
export default function PropsFavoriteButton({ player, selectedStat, avg }) {
  ...
  return (
    <button
      ...
      className={`flex h-7 w-7 flex-shrink-0 cursor-pointer items-center justify-center rounded transition-colors focus:outline-none disabled:cursor-wait disabled:opacity-50 md:h-9 md:w-9 ${favorite ? "text-orange-400" : "text-slate-500 hover:text-slate-200"}`}
    >
